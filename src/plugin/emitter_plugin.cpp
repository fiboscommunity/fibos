...
static emitter_plugin* _emitter_plugin = nullptr;
...
public:
    ...
    template <typename T>
    fc::variant to_variant_with_abi(chain_plugin* chain_plug, const T& obj)
    {
        ...
        abi_serializer::to_variant(
            obj, pretty_output, [&](account_name n) {
                if (n.good()) {
                    try {
                        const auto& a = chain.get_account(n);
                        if (abi_def abi; abi_serializer::to_abi(a.abi, abi))
                            return std::optional<abi_serializer>(abi_serializer(std::move(abi), yield));
                    }
    ...
    {
        try {
            fc::variant trans = to_variant_with_abi(chain_plug, *data);
    ...
    static v8::Local<v8::Value> js_object(fibjs::Isolate* isolate, const std::vector<char>& var)
    {
        ...
        try {
            ...
            obj->Set(context, isolate->NewString("block_num"), v8::BigInt::New(isolate->m_isolate, block.block_num())).IsJust();
            obj->Set(context, isolate->NewString("id"), isolate->NewString(block.calculate_id().str())).IsJust();

            v8::Local<v8::Object> block_obj = v8::Object::New(isolate->m_isolate);
            block_obj->Set(context, isolate->NewString("timestamp"), isolate->NewString(fc::time_point_sec(block.timestamp).to_iso_string())).IsJust();
            block_obj->Set(context, isolate->NewString("producer"), isolate->NewString(block.producer.to_string())).IsJust();
            block_obj->Set(context, isolate->NewString("previous"), isolate->NewString(block.previous.str())).IsJust();

            obj->Set(context, isolate->NewString("block"), block_obj).IsJust();
        } catch (std::exception& e) {
...
void emitter_plugin::plugin_initialize(const variables_map& options)
{
    try {
        ...
        isolate->sync([isolate]() -> int { return eosio::emitter_plugin_impl::js_handler(isolate); });
